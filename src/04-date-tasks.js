...
function timeSpanToString(startDate, endDate) {
  ...
  const hour = String(Math.floor(differ / (1000 * 60 * 60))).padStart(2, '0');
  const min = String(Math.floor((differ / (1000 * 60)) % 60)).padStart(2, '0');
  const sec = String(Math.floor((differ / 1000) % 60)).padStart(2, '0');
  const millisec = String(Math.floor(differ % 1000)).padStart(3, '0');
...
function getDay(day, isLeap) {
  ...
  return `${month[monthIndex]}, ${monthDay}`;
}
