...
function isCreditCardNumber(ccn) {
  const num = ccn.toString().split('').reverse().map(Number);
  return (
    num.reduce((sum, digit, idx) => {
      let modNum = digit;
      if (idx % 2 === 1) {
        modNum *= 2;
        if (modNum > 9) modNum -= 9;
      }
      return sum + modNum;
    }, 0) % 10 === 0
  );
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((char) => {
    if ('({[<'.includes(char)) {
      ...
      if (stack.pop() !== scobs[char]) {
        status = false;
      }
    }
  });
  return status && stack.length === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const minLen = Math.min(...splitPathes.map((p) => p.length));
  let comPathes = '';
  for (let i = 0; i < minLen; i += 1) {
    const elem = splitPathes[0][i];
    if (splitPathes.every((path) => path[i] === elem)) {
      comPathes += `${elem}/`;
    } else {
      break;
    }
...
function getMatrixProduct(m1, m2) {
  const a = m1.length;
  const a1 = m1[0].length;
  const b1 = m2[0].length;
  const result = Array.from({ length: a }, () => Array(b1).fill(0));
  for (let i = 0; i < a; i += 1) {
    for (let j = 0; j < b1; j += 1) {
      for (let f = 0; f < a1; f += 1) {
